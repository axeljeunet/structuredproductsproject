...
public:
    PricingManager& pricer;
    explicit GrpcPricerImpl(PricingManager& pricer) : pricer(pricer) {}
    ...
    Status PriceAndDeltas(ServerContext *context, const PricingInput *input, PricingOutput *output) override {
        ...
        PnlVect* deltaStdDev = pnl_vect_create_from_zero(past->n);
        Parser* parser = new Parser(input->json().c_str());
        pricer.updateData(parser);
    ...
    Status Heartbeat(ServerContext *context, const Empty* input, ReqInfo *output) override {
        output->set_domesticinterestrate(pricer.globalModel->domesticInterestRate.interestRate);
        output->set_relativefinitedifferencestep(0.0);
        output->set_samplenb(0);
        return Status::OK;
    }
...
void RunServer() {
    ...
    PricingManager* pricer = new PricingManager();
